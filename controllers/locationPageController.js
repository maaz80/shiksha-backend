...
export const getLocationPage = async (req, res) => {
     try {
          const data = await LocationPage.findOne().sort({ createdAt: -1 });
          res.json(data);
     } catch (err) {
...
export const createLocationPage = async (req, res) => {
     try {
          const body = req.body;
          ...
          const item = new LocationPage(parsed);
          await item.save();
...
export const updateLocationPage = async (req, res) => {
     try {
          const { id } = req.params;
          ...
          const updated = await LocationPage.findByIdAndUpdate(
               id,
               parsed,
               { new: true }
          );
...
export const deleteLocationPage = async (req, res) => {
     try {
          const { id } = req.params;
          await LocationPage.findByIdAndDelete(id);
